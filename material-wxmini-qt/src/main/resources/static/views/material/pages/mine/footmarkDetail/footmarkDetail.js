...
Page({
    ...
    collectSuccess: function () {
        var isAddRes = 0;
        var that = this;
        if (this.data.currentCase.isCollection == 0) {
            isAddRes = 1;
        }
        this.data.currentCase.isCollection = isAddRes;
        this.setData({
            currentCase: this.data.currentCase
        });
        var collectionObj = {
            collectionCls: 'CASE', //收藏分类(案例：CASE，色卡：COLOR_MAP)
            relationId: this.data.relationId, //关联分类ID
            collectionTitle: this.data.currentCase.footmarkTitle, //收藏标题
            imageUrl: this.data.currentCase.imageUrl, //封面图
        ...
        commservice.request({
            ...
            success: function (res) {
                if (res.data.success) {
                    ...
                    prevPage.setData({
                        currentCaseId: that.data.currentCaseId,
                        isCol: isAddRes
                    })
                    that.setData({
    ...
    downloadImg: function () {
        ...
        var imgUrl = this.data.imageHost + this.data.currentCase.orgImageUrl;
        wx.downloadFile({
